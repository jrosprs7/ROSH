...
- Version policy (latest user instruction): advance both regional HTML versions together for each delivered files/program update: v.01, v.02, v.03, and so on. Current published baseline is v.02; the next program update must be v.03. Increment once per delivered update, not for every intermediate edit or test rebuild. Update builder output names, Download updated HTML names, tests, README links and any displayed version together. Preserve prior releases through Git history; keep only the current release as the active linked calculator. Preserve administrator settings when migrating. Recording this policy alone does not create a new program release.
